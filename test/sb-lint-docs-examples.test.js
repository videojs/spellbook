...
describe('sb-lint-docs-examples', function() {
  beforeEach(function() {
    this.helper = new Helper();
    this.config = this.helper.setup();
    this.bin = path.join(__dirname, '..', 'src', 'sb-lint-docs-examples') + ' ';
  });
  afterEach(function() {
    this.helper.cleanup();
  });
  ...
  it('should lint default files with no args', function(done) {
    shelljs.exec(this.bin, function(code, stdout, stderr) {
      ...
      assert.equal(stdouts.length, 5, 'should stdout 5 lines');
      assert.equal(stderrs.length, 0, 'should stderr nothing');
