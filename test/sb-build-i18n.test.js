...
describe('sb-build-i18n', function() {
  beforeEach(function() {
    this.helper = new Helper();
    this.config = this.helper.setup();
    this.bin = path.join(__dirname, '..', 'src', 'sb-build-i18n') + ' ';
  });
  afterEach(function() {
    this.helper.cleanup();
  });
  ...
  it('should build default files with no args', function(done) {
    ...
    shelljs.exec(this.bin, function(code, stdout, stderr) {
      ...
      assert.ok((new RegExp(path.join(config.dist, 'i18n'))).test(stdouts[0]), 'should contain dist');
      done();
    });
  });
  ...
  it('should different src file to default dist if passed in', function(done) {
    ...
    shelljs.mv(path.join(config.src, 'i18n'), newsrc);
