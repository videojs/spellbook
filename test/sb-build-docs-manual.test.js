...
describe('sb-build-docs-manual', function() {
  beforeEach(function() {
    this.helper = new Helper();
    this.config = this.helper.setup();
    this.bin = path.join(__dirname, '..', 'src', 'sb-build-docs-manual') + ' ';
  });
  afterEach(function() {
    this.helper.cleanup();
  });
  ...
  it('should build default files with no args', function(done) {
    ...
    shelljs.exec(this.bin, function(code, stdout, stderr) {
      ...
      assert.ok((new RegExp(path.join(config.dist, 'docs', 'manual'))).test(stdouts[0]), 'should contain dist');
      assert.ok((new RegExp(path.join(config.src, 'docs'))).test(stdouts[0]), 'should contain js src');
  ...
  it('should different src file to default dist if passed in', function(done) {
    ...
    shelljs.mv(path.join(config.src, 'docs'), newsrc);
